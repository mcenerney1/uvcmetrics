...
class dirtree_datafiles( basic_datafiles ):
   def __init__( self, opts, obs=None, pathid=None):
      ...
      if obs == None:
         if pathid == None:
            root = os.path.expanduser(self.opts._opts['path'][0])
         else:
            root = os.path.expanduser(self.opts._opts['path'][pathid])
      else:
         root = os.path.expanduser(self.opts._opts['obspath'][0])
...
#         print filt
      ...
      basic_datafiles.__init__(self, self.opts)

      # assume one directory per dirtree_datafiles call
      # otherwise, this breaks the uvcdat code
      # there is no reaason to keep that assumption, but it is required right now.

      self._getdatafiles(root, filt)
   ...
   def _getdatafiles( self, root, filt ):
      ...
      for dirpath,dirname,filenames in os.walk(root):
         ...
         return self.files
   def short_name(self):
      if(self.opts._opts['dsname']) != None:
         return self.opts._opts['dsname']
      else:
